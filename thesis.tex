...
for calling the LP callback methods during the course of the simulation. The event dispatcher can
be configured for sequential simulation or a time warp simulation. The sequential simulation
does not depend on any other components and contains only a single list of unprocessed events.
The time warp event dispatcher on the other hand depends on multiple components that implement
specialized algorithms for event scheduling, state saving, cancellation, GVT, termination,
interprocess communication, and statistics. The main components and how they depend on eachother
are illustrated in figure \ref{warped2_architecture}.
...
mechanism of the individual LP's such as rolling back LPs and fossil collecting LPs whereas
...
cancellation technique. The \texttt{OutputManager} base class provides a set of methods that a derived
...
for state saving and state restoration. Similar to the output manager, the state manager is
based on a \texttt{StateManager} base class. The derived class provide methods for saving the
state of an LP, restoring the state of an LP, and fossil collecting old states. Currently,
\textsc{warped2} only implements periodic state saving.
...
variable must be passed to this macro as well as \texttt{cereal::base\_class<warped::Event>(this)} to
...
be registered using the \texttt{WARPED\_REGISTER\_POLYMORPHIC\_SERIALIZABLE\_CLASS} macro. A
basic example of an event implementation is shown below in listing \ref{event_example}.
...
but each machine contains two sockets so that 8 cores are available.
A summary of the architectural features and runtime systems for both machines are summarized in
table \ref{x86_platform} in appendix \ref{x86_nodes_and_clusters}.

\subsection{Experiments on x86 SMP Nodes}

%% Increasing LTSF Queues, LTSF Queue Partitioning, State Saving Period (Performance), Spinlocks

%% Fossil Collection, State Saving Period (Memory), Memory Allocation

\subsection{Experiments x86 Clusters}
...
events means that there will be very little synchronization overhead between threads and funnelled
communication will only increase the communication latency, and thus increase rollbacks. As
remote communication increases, however, the synchronization costs of worker threads increases
and funnelling all communication to the manager thread can help to reduce synchronization and
increase performance but the latency and rollbacks will both increase. Aggregating messages
with funnelled communication can decrease the rollbacks even though latency is still increased
since there is a chance that the anti-messages are sent with their positive counterpart.
...
should be minimized as much as possible to avoid synchronization and communication performance costs
...
to aggregate for each receiver process. The following two sections will provide any overview
of supported partitioning schemes and message aggregation, respectively.
...
cross-partition communication is also minimized. Profile-Guided partitioning should be used if
...
\chapter[\textsc{warped2} Data Structures]{\textsc{warped2} Data Structures and Organization}\label{warped2_ds}
...
LTSF queue if available. If the event is positive then it is processed normally, the state of
the LP is saved, and new events are sent to other LPs. The event is then moved into the processed
queue and a new event is scheduled into the LTSF queue. To summarize, the worker thread processing
loop is shown in psuedocode in algorithm \ref{warped2_processing}.
...
of LTSF queues with 10 worker threads. For the analysis of LTSF queues, speedup will be defined
as:
...
performances and multiple simulation models can be compared.
...
be minimized further. Access to the LTSF queue will always be necessary to obtain the next event
and reschedule events and since the LTSF queue is only accessed while sending an event if the
event is a straggler, it will save a future access to the LTSF queue when the straggler is
scheduled while also preventing a rollback.
...
Figure \ref{ssp_analysis_smp_performance} shows the speedup over copy state saving
and average number of coast forwarded events per rollback as the state saving period is increased
for all models on a single SMP machine. $Speedup_{CSS}$ is defined as:

$$ Speedup_{CSS} = runtime_{1}/runtime_{N} $$

\noindent
where $runtime_{1}$ is the runtime with a state saving period of 1 and $runtime_{N}$ is runtime
with the state saving period of $N$. The coast forward rate and rollback rate are also used to
analyze state saving and are related by:
...
increases to the point where it will cancel out the reduced time from reduced copying. An interesting
result is that even though the rollback rate increases for lower state saving periods, a speedup
is still observed. The coast forwarded rate depends on both the total rollback rate and the
...
critical section is quick. All of the accesses to the LTSF queues are quick in \textsc{warped2}
so the locks should only held for a short period of time.
...
after the start of the algorithm are considered to be "red" messages and are recorded.
When the messages count reaches zero a global reduction is performed on the local minimum
timestamp of all processes while also taking into account the red messages timestamps. The reduction
yields a GVT value for all processes. This algorithm is efficient when time warp simulation are
run on large supercomputing platforms like the blue gene machine which perform collective operations
very quickly.
...
\SetKwBlock{Receive}{receiveRemoteEvent}{end}
    ...
    \Send{
        ...
        \If{$localGVTFlag_i > 0$ and ($workerThread_i$ hasn't reported local min)}{
...
are used to keep track of message that are sent that carry the initial color. That means that
...
vector counters is that vector counters require that communication is stopped after receiving a
control token until all known transient messages that are bound for the receiving process are
received. With scalar counters, this is not possible because all counts are combined into a single
counter. However, with scalar counters it is possible that more than two rounds of the control
token will be necessary. This is usually not a problem in practice though since two rounds is
usually sufficient. Also, each process must maintain a counter for both white messages \emph{and}
red messages so that counters can be consistent between multiple runs of the algorithm. The roles
of the colors must also be switched between successive runs.

A GVT token will be sent by the manager thread when a local minimum has been reported by all
worker threads and the local GVT calculation will begin either when a token is received or
when the initiator process determines that is should start. The initiator process is the process
with PID of 0. Due to the asynchrony of this algorithm, there are two phases of the algorithm,
one phase to check the state of the algorithm and progress it further and the other phase is
to receive the token and update the state. The two phases are put into psuedocode in algorithm
\ref{gvt_manager_asynchronous}.

\begin{algorithm}
\DontPrintSemicolon
\SetKwFunction{SendToken}{SendToken}
\SetKwBlock{ProgressGVTa}{progressGVT}{end}
\SetKwBlock{ReceiveToken}{receiveToken}{end}

    \ProgressGVTa{
        \If {$needLocalGVT = true$}{
            $needLocalGVT \gets false$\;\;

            \uIf{($localGVTFlag$ equals 0) and (haven't started local GVT)}{
                $color \gets color_{final}$\;
                $ts_{min} \gets \infty$\;
                $clock_{min} \gets \infty$\;
                $msgcount \gets msgcount_{initial}$\;
                $msgcount_{initial} \gets 0$\;
            } \ElseIf {($localGVTFlag = 0$) and (started local GVT)}{
                $localMin = \infty$\;
                \ForEach{worker thread, i}{
                    $localMin \gets \min{(localMin, localMin_i)}$\;
                }\;

                \uIf{$processCount > 1$}{
                    \SendToken{$i, (i+1) \bmod N, localMin, \min{(ts_{min}, clock_{min})}, msgcount$}\;
                } \Else {
                    $gvtApprox \gets localMin$\;
                }
            }
        }
    }\;
    ...
    \ReceiveToken{
        \uIf{$PID \neq 0$}{
            ...
            Accumulate $ts_{min}$ and $msgcount$\; 
            $clock_{min} \gets \min{(clock_{min}, mclock)}$\;\;

            $localGVTFlag \gets numWorkerThreads$\;
            $needLocalGVT \gets true$\;
        } \Else {
            \uIf {$mcount \neq 0$} {
    ...
    \caption{Asynchronous GVT: Manager Thread}\label{gvt_manager_asynchronous}
\end{algorithm}

The asynchronous algorithm desribed above works well in cases where the remote communication is
relatively low but has been shown to be problematic under heavy remote communication. The root
of the problem is that the GVT token must compete with remote events that must be sent and and
received by all worker threads. During the course of the GVT calculation the manager thread cannot
stop sending and receiving these events and the token is slowed down significantly. To get around
this constraint, users can either increase the number LPs or decrease the number of worker threads
to reduce the network load.

\subsubsection{Synchronous}

The synchronous GVT algorithm is a simple extension of the allreduce algorithm implemented
...
the processes are not considered to be colored and any event sent by the worker threads before
reporting a local minimum are recorded on the send side rather than the receive side.
...
When the worker threads report their local minimum, they must wait at TWO thread barriers. Both
barriers are needed so that the manager thread can ensure that no more remote events are sent.
...
release the worker threads and indicates that the manager thread has completed the GVT calculation.  
...
manager thread has more LPs to fossil collect or if it had remote messages to send. The time
taken to fossil collect with more LPs could interfere with sending and receiving remote messages
or with calculating the GVT.
...
task scheduling. An imbalance can also be observed in the number of rollbacks as shown in
figure \ref{allocator_analysis_efficiency} which shows the efficiency with ptmalloc2 and tcmalloc.
...
that are used by the allocators. With both ptmalloc2 and tcmalloc, a thread-local cache is used
to allocate memory from but with ptmalloc2, freed memory cannot move from one local cache to
another and must be obtained from the OS. This alone, can create an imbalance in processing
speed among threads and trigger lots of rollbacks.
...
10 worker threads. Also the memory measurements are taken from the actual memory used by a
...
of a problem than a small delay in communication latency. In general, though, avoiding remote
communication at all costs by partitioning and aggregating messages further decreases these
communication latency overheads and rollbacks, and thus increases performance.
...
such as replacing the default ptmalloc2 allocator with tcmalloc and jemalloc and replacing blocking
...
communication between worker threads, any optimization that reduces synchronization to shared
data structures is usually better. The two coincide, however, when figuring out how worker threads
should use the message passing interface in a thread safe manner, which makes interprocess
communication especially difficult for hybrid communication applications.

For a single SMP node, increasing the numbe of LTSF queues, LTSF queue partitioning,
...
For reducing communication overheads, partitioning and 

%% More about manager thread and communication

\subsection{Scalability versus Scalability}

There is also a fundamental tradeoff between scalability and performance. Message passing between
multiple processes will be a bottleneck but is necessary for scaling up. Also, for better scalability,
a fossil collection method which is carried out by the worker threads is more suitable so that
remote communication can be completed with minimal latency on the manager thread. Although this
method may slow down the maximum event rate for a single node, it will actually allow a larger
event rate to be achieved by scaling up.

The tradeoff between performance and memory consumption is also apparent

\section{Suggestions for Future Work}

\subsection{Scaling Simulations}

It is still largely unknown on what will happen as the simulations are scaled up to larger
multi-core processors and/or larger clusters and whether a strong scaling or a weak scaling
approach should be taken. Scaling will be particularly tricky with multi-core clusters that
have a large number of cores per processor. 

\subsection{Memory Management}

%% Custom in-kernel allocator
