...
of \textsc{warped2} are discussed. All chapters except for chapter \ref{big_little_platform} will
contain experimental results for only x86 SMP nodes and clusters and will be targeted towards
understanding and determining the best design options and configurations. Chapter \ref{big_little_platform}
...
The block size is the number of contiguous LPs that are allocated to a partition at a time.
The block size can be between 1 and $NumLPs$/$NumProcesses$ where the former is a true round-robin
partitioning scheme and the latter is a block partitioning scheme.

A block size in between the extreme values can be used to 
...
\begin{figure}
  \begin{center}
    ...
    Round-Robin with blocksize = $NumLPs$/$NumProcesses$ \\
  \end{center}
  \caption{Round Robin Partitioning}
\end{figure}
...
%% Explanation of why message aggregation works!!
...
%% Size based versus Number of messages.
An alternative way of implementing message aggregation would be to combine messages until a
certain size is reached rather than a number of messages.

Another alternative approach, that would minimize the size of the aggregated message would be to
use a special character to seperate messages so that the header is not necessary.
...
\begin{figure}
  ...
  \begin{minipage}{.5\textwidth}
    ...
    \end{center}
  \end{minipage}
  \caption{Aggregate Messages: PCS Model}
\end{figure}
...
\begin{figure}
  ...
  \begin{minipage}{.5\textwidth}
    ...
    \end{center}
  \end{minipage}
  \caption{Aggregate Messages: Traffic Model}
\end{figure}
...
\begin{figure}
  ...
  \begin{minipage}{.5\textwidth}
    ...
    \end{center}
  \end{minipage}
  \caption{Aggregate Messages: Epidemic Model}
\end{figure}
...
scaled up to larger numbers on an 8 node cluster. 
...
\begin{figure}
  ...
  \begin{minipage}{.5\textwidth}
    ...
    \end{center}
  \end{minipage}
  \caption{Scaling on Cluster: Efficiency and \% Remote Events}
\end{figure}
...
performances and multiple simulation models can be compared. Another measure called the
...
\subsubsection{LTSF Queue Partitioning}
...
Figures \ref{ssp_analysis_smp_performance} and \ref{ssp_analysis_cluster} show the speedup over
copy state saving and the efficiency as the state saving period is increased for all models for
a single SMP machine and an 8 node cluster. $Speedup_{CSS}$ is defined as:
...
critical section is quick. All of these accesses are quick so
...
\SetKwBlock{ReceiveToken}{receiveToken}{end}
    ...
    \ProgressGVTa{
        \If {Local GVT is needed}{
            ...
            } \ElseIf {($localGVTFlag$ equals 0) and (started local GVT)}{
                $localMin = \infty$\;
    ...
    \ReceiveToken{
        \uIf{$ID_process$ does not equal 0}{
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth,keepaspectratio,quiet]{figs/pending_event_set/ltsf_memory_increase.pdf}
  \caption{LTSF Queues: \% Increase in Average Peak Memory}
\end{figure}
