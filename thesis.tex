...
\chapter{Message Passing Communication and Partitioning}
...
The plots in figure \ref{communication_models} summarizes the performance of funnelled, serialized,
and multiple thread communication. For serialized communication, a single global spinlock is
used to protect access to all MPI library calls.

\section{Partitioning}

Partitioning the work in a distributed system is important to minimize latency costs incurred
from interprocess communication. In parallel discrete event simulation, the work is usually
partitioned by the LPs so that each process in the system can process events from a dedicated
subset of LPs. This is realized by using a SPMD programming model, where each process runs exactly
...
within a single process. This will create a much more stable simulation since the communication
latency is much larger than the frequency of computation on each of the local machines and this
disparity increases the chances of rollbacks occuring.

\subsection{Partitioning in \textsc{warped2}}

Currently, \textsc{warped2} supports round-robin partitioning, profile-guided partitioning,
profile-guided partitioning, and custom partitioning. The custom partitioning is currently
not being used in any \textsc{warped2} simulation models.

Round-robin partitioning can be configured with a block size, where the block size determines
how many LPs are allocated to each process at a time. A valid block size must be between 1
and $NumLPs \over NumProcs$, where the former is a true round robin partitioning scheme and the
latter is block partitioning scheme. Profile-guided guided partitioning is based on profile data
that must be collected with a sequential simulation. The profile data contains a count of events
for each communication pair of LPs and the partitioner uses that data to build a weighted graph.
Then, the partitioner uses a graph partitioning tool to create partitions that minimize the cut-set.
A detailed analysis of the partitioning methods will be provided in chapter \ref{experimental analysis}.
...
\begin{table}
    ...
    & & Intel\textsuperscript{\textregistered} Xeon\textsuperscript{\textregistered} X5675 &
        ...
        \multirow{3}{*}{Runtime}
            ...
            & Memory Allocator  & TCMalloc v2.4                 & TCMalloc v2.4     \\
        \hline
    \end{tabular}
...
\begin{figure}
  ...
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio]{figs/state_saving/average_cf.pdf} \\
      Average Coast Forward Events
    \end{center}
  \end{minipage} \hfill
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio]{figs/state_saving/efficiency.pdf} \\
      $Efficiency$ \\
    \end{center}
  \end{minipage}
...
\begin{figure}
  ...
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio]{figs/fossil_collection/time_fcw.pdf} \\
    \end{center}
  \end{minipage}
...
\begin{figure}
  ...
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio]{figs/pending_event_set/ltsf_memory.pdf} \\
      Average Peak Memory \\
    \end{center}
  \end{minipage} \hfill
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio]{figs/pending_event_set/ltsf_rollbacks.pdf} \\
      Total Rollbacks \\
    \end{center}
  \end{minipage}
...
\begin{figure}
\centering
  \includegraphics[width=0.5\textwidth]{figs/pending_event_set/partitioning_speedup.pdf}
  \caption{Speedup with Profile-Guided Partitioning}\label{ltsf_partitioning}
\end{figure}

\subsection{Process Parttioning}
...
\begin{figure}
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio]{figs/memory_allocation/ptm_times.pdf} \\
      PTMalloc2 \\
    \end{center}
  \end{minipage} \hfill
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio]{figs/memory_allocation/tcm_times.pdf} \\
      TCMalloc \\
    \end{center}
  \end{minipage}
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{figs/memory_allocation/tcmalloc_speedup.pdf}
...
\begin{figure}
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio]{figs/big_little/pcs_rr.pdf} \\
      PCS \\
    \end{center}
  \end{minipage} \hfill
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio]{figs/big_little/traffic_rr.pdf} \\
      Traffic \\
    \end{center}
  \end{minipage}
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio]{figs/big_little/epidemic_rr.pdf} \\
      Epidemic \\
    \end{center}
  \end{minipage} \hfill
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio]{figs/big_little/airport_rr.pdf} \\
      Airport \\
    \end{center}
  \end{minipage}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth,keepaspectratio]{figs/big_little/os_partitioning_times.pdf}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=\textwidth,keepaspectratio]{figs/big_little/os_partitioning_rbs.pdf}
\end{figure}
...
\begin{figure}
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio]{figs/big_little/fetchadd.eps} \\
      Fetch and add \\
    \end{center}
  \end{minipage} \hfill
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio]{figs/big_little/fetchsub.eps} \\
      Fetch and subtract \\
    \end{center}
  \end{minipage}
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio]{figs/big_little/sharedptr_copy.eps} \\
      Shared\_ptr copy \\
    \end{center} \hfill
  \end{minipage}
\end{figure}
...
%% Can we lower contention with fewer LTSF queues so that rollbacks are minimized???

%% MPI latency and MPI lock contention
