...
%% I NEED TO FIX THIS SECTION UP MORE... HAVING SOME TROUBLE EXPLAINING THIS.
...
\texttt{TimeWarpKernelMessage} base class which will allow all message types to be stored
in the same container data structures.

The receive callback is defined in the class that sends the message just as any other method.
The \texttt{WARPED\_REGISTER\_MSG\_HANDLER} macro can be used to register the message types
and recieve callback as long as the communicating class contains a data member that is a pointer
the \texttt{CommunicationManager} and is named \texttt{comm\_manager\_}. 
