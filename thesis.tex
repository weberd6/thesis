...
\begin{table}
    ...
    \hline
    & & Intel Core i7-4770 & Intel Xeon E5410 \\ [0.5ex]
        \hline\hline
        \multirow{8}{*}{Processor}
            & ISA           & x86\_64   & x86\_64   \\
            & \# Cores      & 4         & 8 (2x4)   \\
            & \# Threads    & 8         & 8         \\
            & Frequency     & 3.4 GHz   & 2.33 GHz  \\
...
The first aspect of \textsc{warped2} that will be analyzed will be the state 
...
Therefore, if the event is an anti-message, the positive counterpart is assumed to be in the LPs
...
the state of the LP is saved, and new events are sent to other LPs. The event is then inserted
...
than the currently scheduled event, then the new event is scheduled, thus preventing and
...
and the number of rollbacks can increase significantly.

To make matters more complicated this
tradeoff can vary as the event processing algorithms and LTSF Queue data structures in
\textsc{warped2} evolve.
...
needed to process a larger number of rollbacks. This can be explained by looking back at the
event processing loop back in algorithm \ref{warped2_processing}. The lines with a number on the
margin indicate possible access to the LTSF queue. For every event the LTSF queue is accessed
at least 3 times: \begin{inparaenum}[(1)] \item obtaining the next event; \item sending new
events; and \item rescheduling new events \end{inparaenum}. As the number of LTSF Queues 
...
that quick access to the LTSF queues is necessary for the best performance. This section
describes the use of spinlocks as an alternative to the standard blocking mutexes.

\subsection{Spinlocks}

Traditionally, mutexes are implemented in such a way that if a thread cannot acquire the
lock, the thread is put to sleep and rescheduled for execution at a later time. If the locks
are usually held for a short period, yielding the processor can waist more time than is necessary
to wait for the lock to be free. \textsc{warped2} is a perfect example of this type of behavior
since accesses to the LTSF queues only perform very simple and fast operations.
...
fairness, and generally have good performance for a small number of threads\cite{lockless-10}.
The psuedocode below in algorithm \ref{ticket_lock} describes the acquisition and release
procedures for a typical ticket lock implementation which is also how it is implemented in
\textsc{warped2}.
...
%% Analysis of spinlocks in warped2
...
\begin{figure}[H]
    ...
    \caption{Partitioning in \textsc{warped2}}\label{partitioning_fig}
\end{figure}
