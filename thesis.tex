...
of \textsc{warped2} are discussed. All chapters
%%except for chapter \ref{big_little_platform}
will contain experimental results for only x86 SMP nodes and clusters and will be targeted towards
understanding and determining the best design options and configurations.
%%Chapter \ref{big_little_platform}
%%will contain seperate experiments and observations on the ARM big.LITTLE HMP platform.
...
\subsection{Experiments ARM big.LITTLE Nodes}
...
\chapter{Interprocess Communication} \label{partitioning_communication}
...
a lot remain on the same node and minimize network communication in clusters. Currently,
\textsc{warped2} supports three partitioning schemes: \begin{inparaenum}[(1)] \item Round-Robin,
\item Profile-Guided, and \item Custom \end{inparaenum}. 

\paragraph{Round-Robin} partitioning is a simple partitioning scheme where $block\_size$ number
of LPs are allocated to each process at a time and in order of increasing process id.
$block size$ can be between 1 and $NumLPs$/$NumProcesses$ where the latter is simply known as
block partitioning. From now on, a $block\_size$ of 1 will also be called simply round-robin.
...
partitioning scheme would work with different values for $block_size$, where different partitions
...
profile-guided partitioning schemes.
...
\textsc{warped2} implements both a synchronous and asynchronous GVT algorithms. For the
...
The details of how the the Mattern's algorithm implementation in \textsc{warped2} is detailed below as well as how
...
the counters from each process just like with GVT.
...
A comparison of the event rates for ptmalloc2 which is the default allocator for GLIBC and tcmalloc
is shown in figure \ref{allocator_analysis_eventrate}.
...
%% Paragraph on cluster best practices and reduction of remote events

%% Paragraph on SMP best practices and decreasing contention

\section{Detailed Conclusions}

%% Scalability versus Performance

%% Performance versus Memory consumption

%% What happens with larger simulation models

\section{Suggestions for Future Work}

%% Lock contention
%% Scaling up the number of LPs/Nodes
%% Memory Management
