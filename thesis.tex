...
%% DOUG: i believe it would be a good idea for you to include a complete simulation model somewhere
%% in this thesis.  perhaps as an appendix.  more specifically as i was studying this chapter, i
%% found myself needing a more complete example to study (i'm especially thinking about other
%% researchers).  let's talk about this
...
%% DOUG: please check for accuracy.
...
%% DOUG: take a look at this next paragraph and compare it to the way you had written it.
%% specifically the second sentence has been broken into two sentences.  the changes are small, but
%% the reading is much more clear and actually more factual.  please try to go through your thesis
%% and cleanup like this.  i will be away from it until late wed.
...
\begin{enumerate}
    \item The \texttt{initializeLP} method is called to perform any initializations of an LP that
        ...
        but are not limited to \begin{inparaenum}[(1)] \item registering a random number generator
        with the \textsc{warped2} kernel and \item producing a set of initial events to be sent.
        \end{inparaenum}
...
%% hmmmm....thinking this over, i'm not sure we need both subsections.  i think this portion of the
%% thesis is really more about the conceptual structure of the implementation.....will think on it.
...
\begin{table}[H]
    ...
    \begin{tabular}{| c | c | c | c |}
        ...
        \hline
        Aggregate Messages & Event Rate & Figures \ref{aggregate_eventrate}, \ref{aggregate_epidemic} & E5410 \\ 
        \hline
        Aggregate Messages & Efficiency & Figures \ref{aggregate_efficiency}, \ref{aggregate_epidemic} & E5410 \\ 
        \hline
        Aggregate Messages & Rollback Length & Figures \ref{aggregate_rblength}, \ref{aggregate_epidemic} & E5410 \\ 
        \hline
...
communication model where worker threads handle all remote communication.
In general, more performance is closely tied with a lower percentage of remote events and a
higher efficiency.
...
split at the receiver.  By coarsening the granularity, aggregating messages can also increase
the efficiency of simulations by lowering rollbacks.
...
The plots in figures \ref{aggregate_eventrate} and \ref{aggregate_efficiency} show how various
message aggregate sizes affect event rate and efficiency for the PCS and Traffic models.  The
results are also plotted for different state saving periods since different state saving period
has a significant impact on these measures.
...
The results for PCS and Traffic indicate that just a small number of aggregated messages can
have a significant impact on increasing performance and efficiency as long as the state saving
period is not too high.  

With a larger state saving period, the performance drops off much faster with an increasing
number of aggregated messages than with a small state saving period.  This is probably due the
increased amount of time that is spent restoring the state of the LP back to a its proper state.
...
\begin{figure}
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio,quiet]{figs/partitioning_communication/aggregate_pcs_rblength.pdf} \\
      PCS \\
    \end{center}
  \end{minipage}%
  \hfill
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio,quiet]{figs/partitioning_communication/aggregate_traffic_rblength.pdf} \\
      Traffic \\
    \end{center}
  \end{minipage}
  \caption{Aggregate Messages: Rollback Length}\label{aggregate_rblength}
\end{figure}
...
\begin{figure}
  ...
  \begin{minipage}{.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio,quiet]{figs/partitioning_communication/aggregate_epidemic_rblength.pdf} \\
      Rollback Length \\
    \end{center}
  \end{minipage}
...
algorithm based on Mattern's "sticky flags" algorithm\cite{mattern-91}.  Just like the GVT
...
%% Something about message aggregation and rollback profiling
